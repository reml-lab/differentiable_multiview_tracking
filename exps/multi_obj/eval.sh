...
CONFIGDIR=exps/multi_obj
BASEEXPDIR=exps/multi_obj/logs
mkdir -p $BASEEXPDIR
...
mkdir -p $EXPDIR
cp $SRCROOT/checkpoints/multi_obj_stage0.pt $EXPDIR/checkpoint.pt
...
mkdir -p $EXPDIR
cp $SRCROOT/checkpoints/multi_obj_stage1.pt $EXPDIR/checkpoint.pt
$RUN run.py $CONFIGDIR/eval_cv.py $EXPDIR
...
mkdir -p $EXPDIR
cp $SRCROOT/checkpoints/multi_obj_stage2_ctrv.pt $EXPDIR/checkpoint.pt
$RUN run.py $CONFIGDIR/eval_ctrv.py $EXPDIR

EXPDIR=$BASEEXPDIR/stage2_cv
mkdir -p $EXPDIR
cp $SRCROOT/checkpoints/multi_obj_stage2_cv.pt $EXPDIR/checkpoint.pt
$RUN run.py $CONFIGDIR/eval_cv.py $EXPDIR
