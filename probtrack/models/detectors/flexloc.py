...
class FlexLocGeospatialDetector(GeospatialDetector):
    def __init__(self, 
                 detector_cfg=None, 
                 bbox_pipeline=[],
                 scenarios=None,
                 ...
                 adapter_cfg=dict(type='FlexLocAdapter'),
    ...
    def forward_adapter(self, embeds, scenario=0, node_str='node_1'):
        ...
        node_pos = proj.get_parameter_vector().T
        embeds = self.adapter(embeds, node_pos)
        return embeds[:,0]

    def detection_loss(self, dets, gt, scenario=0, node_str='node_1'):
        num_objs = len(gt['obj_position'])
        proj = self.projs[scenario][node_str]
        node_pos = proj.get_parameter_vector().T #1 x 9
        view_embeds = self.adapter(dets.embeds, node_pos)[0]
    ...
    def detection_test(self, dets, bbox_pipeline=None, scenario=0, node_str='node_1'):
        embeds = self.forward_adapter(dets.embeds, scenario, node_str)
        det_means, det_covs = self.output_head(embeds)
        det_normals = to_torch_dist(det_means, det_covs, device='cuda')
